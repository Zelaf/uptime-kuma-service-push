...
write_script() {
 ...
 if systemctl is-active --quiet ${args[service]}; then
   curl -G ${args[url]%%'?'*}?status=up --data-urlencode "msg=${args[service]} - running"
 else
   curl -G ${args[url]%%'?'*}?status=down --data-urlencode "msg=${args[service]} - not running. Log: \$(journalctl -xeu ${args[service]} | tail -n5)"
 fi
EOF
  )
